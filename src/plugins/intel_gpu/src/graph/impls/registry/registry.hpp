...
#define OV_GPU_GET_INSTANCE_2(prim, impl_type, shape_types, verify_callback)        \
    std::make_shared<cldnn::ImplementationManagerLegacy<cldnn::prim>>(              \
        std::dynamic_pointer_cast<cldnn::ImplementationManagerLegacy<cldnn::prim>>( \
            GET_INSTANCE(prim, impl_type, shape_types))                             \
            .get(),                                                                 \
        verify_callback),
...
struct Registry {
    static const std::vector<std::shared_ptr<cldnn::ImplementationManager>>& get_implementations() {
        static_assert(cldnn::meta::always_false<PrimitiveType>::value,
                      "Only specialization instantiations are allowed");
