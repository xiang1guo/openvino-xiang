...
struct scaled_dot_product_attention_graph_onednn : typed_primitive_onednn_graph_impl<scaled_dot_product_attention> {
    ...
    void build_graph(const engine& engine, const kernel_params_t& sdpa_param) {
        ...
        auto dtype_inter = logical_tensor::data_type::f32;
        ...
        logical_tensor matmul_qk_out{lt_id++, dtype_inter, qk_output_shape, logical_tensor::layout_type::strided};
        ...
        logical_tensor scaled_qk_out{lt_id++, dtype_inter, qk_output_shape, logical_tensor::layout_type::strided};
        ...
        if (has_attn_mask) {
            ...
            logical_tensor masked_qk_out{lt_id++, dtype_inter, qk_output_shape, logical_tensor::layout_type::strided};
...
protected:
    std::unique_ptr<primitive_impl> clone() const override {
        return std::make_unique<scaled_dot_product_attention_graph_onednn>(*this);
    }
        ...
        for (size_t i = 0; i < query_shape.size(); ++i) {
            if (query_shape[i].is_static() && key_shape[i].is_static() && value_shape[i].is_static()) {
                if (query_shape[i].get_length() > key_shape[i].get_length()) {
                    ...
                    config.kv_group_size = query_shape[i].get_length() / key_shape[i].get_length();
                }
            }
        }
...
public:
    ...
    static std::unique_ptr<primitive_impl> create(const scaled_dot_product_attention_node& arg,
        ...
        return std::make_unique<scaled_dot_product_attention_graph_onednn>(engine,
                                                                             config,
                                                                             sdpa_kernel_params,
