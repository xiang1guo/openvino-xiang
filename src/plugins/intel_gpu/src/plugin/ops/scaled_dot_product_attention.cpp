...
static void CreateScaledDotProductAttentionOp(ProgramBuilder& p,
                                              const std::shared_ptr<op::ScaledDotProductAttention>& op) {
    validate_inputs_count(op, {4, 5});
    const auto inputs = p.GetInputInfo(op);
    const std::string layerName = layer_type_name_ID(op);
    auto order = ov::op::internal::ScaledDotProductAttention::default_order(op->get_output_partial_shape(0).size());
    if (inputs.size() == 4) {
